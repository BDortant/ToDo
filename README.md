...
- **Cross-origin writes are rejected.** Removing CORS stopped other sites *reading* the API, not writing to it: a simple cross-origin `POST` needs no preflight, so any page could still fire `POST /api/todos/cleanup` (which deletes completed todos) and have it run. A middleware now rejects state-changing methods carrying a non-local `Origin`. Requests with no `Origin` at all are allowed through — that is the CLI over curl, where forgery does not apply.
